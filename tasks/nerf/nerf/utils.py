...
class Trainer(object):
    ...
    def test(self, loader, save_path=None, name=None, write_video=True, save_png=True):
        ...
        # --- write video only if write_video True ---
        if write_video:
            all_preds = np.stack(all_preds, axis=0)
            all_preds_depth = np.stack(all_preds_depth, axis=0)
            imageio.mimwrite(os.path.join(save_path, f'{name}_rgb.mp4'),
                            all_preds, fps=2.5, quality=10, macro_block_size=1)
            imageio.mimwrite(os.path.join(save_path, f'{name}_depth.mp4'),
                            all_preds_depth, fps=2.5, quality=10, macro_block_size=1)
